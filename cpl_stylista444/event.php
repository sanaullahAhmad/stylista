...
if(isset($_REQUEST['action']))
	{
		...
		if($_REQUEST['action']=='delete')
			{
				mysql_query("delete from tbl_event  where id=".$_REQUEST['id']."");
	...
	foreach($_POST['all_delete'] as $delete_id)
		{
			...
			mysql_query("delete from tbl_event  where id=".$delete_id."");
...
if(isset($_REQUEST['submit']))
	{
		...
		if($_POST['file_action']=='new')
		{
			...
			//echo "</pre>";
			//exit;
			$sql_ins	=	"insert into tbl_event
			 				set 
					event_name		=	'".mysql_real_escape_string($_POST['intro_title'])."',
					event_description		=	'".mysql_real_escape_string($_POST['description'])."',
                                            event_date          =   '".  mysql_real_escape_string($_POST['event_date']).",'
					status	=	'".$_POST['intro_status']."'";
							 
			 mysql_query($sql_ins);
			//exit;
			 header("location:".$pagename."?msg=Event added Successfully");
		...
		// Edit page code start
			if($_POST['file_action']=='update')
			{
				...
				//exit;
				mysql_query($upd_cat);
				header("location:".$pagename."?page=".$_POST['page_id']."&msg=Event Updated Successfully");
				exit;
...
$total_record = mysql_num_rows(mysql_query("select * from tbl_event"));
...
$sql = "SELECT * FROM tbl_event  ORDER BY id ASC " . $pagination->getLimitSql();
$query=mysql_query($sql);
$order_row=mysql_num_rows($query);

if(isset($_GET['act']))
{
$edit_page = mysql_fetch_array(mysql_query("SELECT * FROM tbl_event where id='".mysql_real_escape_string($_GET['id'])."'"));
}
 ?>
			...
			<div class="content-box"><!-- Start Content Box -->
				...
				<div class="content-box-header">
					...
					<ul class="content-box-tabs">
						<li><a href="#tab1" <?php if(!isset($_GET['act'])) { ?>  class="default-tab" <?php } ?>>Events</a></li> <!-- href must be unique and match the id of target div -->
						<?php if(isset($_GET['act'])) { ?>
						<li><a href="#tab2"   class="default-tab">Modify Event</a></li>
						<?php } else { ?>
						  ...
						  <?php
						   ...
						   while($intro=mysql_fetch_array($res)){ ?>		
								<tr>
									...
									<td>
										<?php 
												echo $intro['event_name'];		
										?>
									</td>
									<td>
										<a href="#" title="title">
											<?php echo substr($intro['event_description'],0,50);?>...
										</a>
									</td>
					...
					<div class="tab-content <?php if(isset($_GET['act'])) { ?> default-tab <?php } ?>" id="tab2">
						...
						<form action="#" method="post">
							...
							<fieldset> <!-- Set class to "column-left" or "column-right" on fieldsets to divide the form into columns -->
								<p>
									<label>Title</label>
										<input class="text-input small-input" type="text" id="intro_title" name="intro_title"  value="<?php if(isset($_GET['act'])) { echo $edit_page['event_name']; } else { echo ""; } ?>" />
								</p>
                                                                <p>
									<label>Date</label>
										<input class="text-input small-input" type="text" id="intro_date" name="event_date"  value="<?php if(isset($_GET['act'])) { echo $edit_page['event_date']; } else { echo ""; } ?>" />
								</p>
								<p>
                                    ...
                                    <div><textarea id="description" name="description"><?php if(isset($_GET['act'])){ echo stripslashes(urldecode($edit_page['event_description'])); } else { echo ""; } ?></textarea>
