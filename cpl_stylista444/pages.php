...
while ($row = mysql_fetch_array($query)) {
    ?>		
                                        <tr>
                                            ...
                                            <td><a href="#" title="title"><?php echo substr(stripslashes(urldecode($row['page_description'])), 0, 40); ?></a></td>
                                            
                                            <td align="center" valign="middle" style="text-align:center;"><?php if ($row['status'] == 1) { ?>
                                                    <a href="<?php echo $pagename; ?>?action=unpub&id=<?php echo $row['id']; ?>&page=<?php if (isset($_REQUEST['page'])) {
                                            echo $_REQUEST['page'];
                                        } else {
                                            echo "1";
                                        } ?>" onClick="return confirm('Are you sure you want to unpublish?')"><img src="proj_resources/images/icons/tick_circle.png" title="UnPublish" alt="UnPublish"></a>

                                                <?php } else if ($row['status'] == 0) { ?>
                                                    <a href="<?php echo $pagename; ?>?action=pub&id=<?php echo $row['id']; ?>&page=<?php if (isset($_REQUEST['page'])) {
                                                echo $_REQUEST['page'];
                                            } else {
                                                echo "1";
                                            } ?>" onClick="return confirm('Are you sure you want to publish?')"><img src="proj_resources/images/icons/cross_circle.png" title="Publish" alt="Publish"></a>
    <?php } ?></td>
                                            <td valign="top">
                                                <!-- Icons -->
                                                <a href="<?php echo $pagename; ?>?act=update&id=<?php echo $row['id']; ?>&page=<?php if (isset($_REQUEST['page'])) {
                                                echo $_REQUEST['page'];
                                            } else {
                                                echo "1";
                                            } ?>" title="Edit"><img src="proj_resources/images/icons/pencil.png" alt="Edit" /></a>
                                                <a href="<?php echo $pagename; ?>?action=delete&id=<?php echo $row['id']; ?>&page=<?php if (isset($_REQUEST['page'])) {
                                                echo $_REQUEST['page'];
                                            } else {
                                                echo "1";
                                            } ?>" onClick="return confirm('Are you sure you want to delete?')"><img src="proj_resources/images/icons/cross.png" alt="Delete" /> </a>
    ...
    <?php $order_counter++;
                    ...
                    <div class="tab-content <?php if (isset($_GET['act'])) { ?> default-tab <?php } ?>" id="tab2">
                        ...
                        <form action="#" method="post">
                            <input type="hidden" name="file_action" value="<?php if (isset($_GET['act'])) {
                                echo "update";
                            } else {
                                echo "new";
                            } ?>">
...
?>" />
                                ...
                                <div><textarea id="description" name="description"><?php if (isset($_GET['act'])) {
                                               echo stripslashes(urldecode($edit_page['page_description']));
                                           } else {
                                               echo "";
                                           }
?></textarea>
                                    <script type="text/javascript">
                                        ...
                                        {
                                            height: '500px',
                                            filebrowserBrowseUrl : '<?php echo $ruadmin ?>ckfinder/ckfinder.html',
                                            filebrowserImageBrowseUrl : '<?php echo $ruadmin ?>admin/ckfinder/ckfinder.html?type=Images',
                                            filebrowserFlashBrowseUrl : '<?php echo $ruadmin ?>ckfinder/ckfinder.html?type=Flash',
                                            filebrowserUploadUrl : '<?php echo $ruadmin ?>ckfinder/core/connector/php/connector.php?command=QuickUpload&type=Files',
                                            filebrowserImageUploadUrl : '<?php echo $ruadmin ?>ckfinder/core/connector/php/connector.php?command=QuickUpload&type=Images',
                                            filebrowserFlashUploadUrl : '<?php echo $ruadmin ?>ckfinder/core/connector/php/connector.php?command=QuickUpload&type=Flash'
                                        }
                                    );
                                        //]]>
                                    </script>
