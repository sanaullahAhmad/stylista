...
if(isset($_REQUEST['action']))
	{
		if($_REQUEST['action']=='pub')
			{
				mysql_query("update  tbl_portfolio_category set status=1 where id=".$_REQUEST['id']."");
				$msg="Publish Successfully";
			}
		if($_REQUEST['action']=='unpub')
			{
				mysql_query("update  tbl_portfolio_category set status=0 where id=".$_REQUEST['id']."");
				$msg="Unpublish Successfully";
			}
		if($_REQUEST['action']=='delete')
			{
				mysql_query("delete from  tbl_portfolio_category  where id=".$_REQUEST['id']."");
...
if($_GET['d']=='up')
	{
		$res=mysql_query("select max(sort1) as sort1 from  tbl_portfolio_category where sort1<".$order."");
		$row=mysql_fetch_array($res);
		$new_rec=$row['sort1'];
		
		
		
		$res1=mysql_query("select * from  tbl_portfolio_category where sort1=".$new_rec."");
		$row1=mysql_fetch_array($res1);
		$old_rec=$row1['id'];
		
		
		
		mysql_query("UPDATE  tbl_portfolio_category SET sort1=".$order." WHERE id=".$old_rec."");
		mysql_query("UPDATE  tbl_portfolio_category SET sort1=".$new_rec." WHERE id=".$id."");
	
	}
if($_GET['d']=='down')
	{
		$res=mysql_query("select min(sort1) as sort1 from  tbl_portfolio_category where sort1>".$order."");
		$row=mysql_fetch_array($res);
		$new_rec=$row['sort1'];
	...
	foreach($_POST['all_delete'] as $delete_id)
		{
			...
			mysql_query("delete from  tbl_portfolio_category  where id=".$delete_id."");
...
if(isset($_REQUEST['submit']))
	{
		...
		{
			$find_max_res=mysql_query("SELECT MAX(sort1) as sort1 FROM  tbl_portfolio_category");
			$find_max_count=mysql_num_rows($find_max_res);
			 if($find_max_count!=0)
				...
				$query=mysql_query("insert into tbl_portfolio_category(sort1,category_name,status) values ('$sort1','$page_title',1)");
		...
		// Edit page code start
			if($_POST['file_action']=='update')
			{
				...
				$query=mysql_query("update  tbl_portfolio_category set 
...
$total_record = mysql_num_rows(mysql_query("select * from  tbl_portfolio_category"));
...
$sql = "SELECT * FROM  tbl_portfolio_category  ORDER BY sort1 ASC " . $pagination->getLimitSql();
$query=mysql_query($sql);
$order_row=mysql_num_rows($query);

if(isset($_GET['act']))
{
$edit_page = mysql_fetch_array(mysql_query("SELECT * FROM  tbl_portfolio_category where id='".mysql_real_escape_string($_GET['id'])."'"));
}
 ?>
...
function valid()
	...
	if(string2.test(document.getElementById("page_title").value)==false)
		...
		inlineMsg('page_title','Please!  Enter Valid Category Title',2);
		return false;
		...
		<div id="main-content"> <!-- Main Content Section with everything -->
			...
			<div class="content-box"><!-- Start Content Box -->
				...
				<div class="content-box-header">
					...
					<h3>Manage Portfolio Categories</h3>
						...
						<table>
							...
							<thead>
								<tr>
								   ...
								   <th width="732">Categories Name</th>
								   <th width="52">Sort</th>
